...
public:
  ...
  int operator()(int *indices) {
    ...
    for(int k=0;k<F::n_components;++k) {
      ...
      /*
	NOTE: This code doesn't look nice as it could, however due lack
	of gatter on SSE/AVX, loading values into SSE/AVX registers
	then performing addition produces many more
	instructions than first adding them then putting (gathering) them
	into register 
      */
...
#pragma omp for reduction(+:accepted)
    for(int s=0;s<BlockType::grid_size();++s) {
      accepted+=update.update_block(BlockType(BlockType::grid(p,s)),block_sweeps);
    }
  }
  return accepted;
} 
