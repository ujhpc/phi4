...
static inline SIVec LCG_step(SIVec z, SIVec A, SIVec C) {
  return (A*z+C);
}
...
public:
  ...
  taus_array(int n):n_generators_(n) {
    seeds_ = (SIVec*)_mm_malloc(pitch*n_generators_*sizeof(SIVec),sizeof(SIVec));
    gen_seeds(121245);
  }
  ...
  inline FVec rand_sym(int i) {
    ...
    return FVec( v1[0], v1[1], v1[2], v1[3],
                 v2[0], v2[1], v2[2], v2[3] );
  #else
  ...
  inline FVec rand(int i) {
    return rand_sym(i)+FVec((Float)0.5);
  }
