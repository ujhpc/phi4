...
public:
  ...
  int operator()(int *indices) {
    ...
    for(int k=0;k<F::n_components;++k) {
      ...
      // NOTE: This code doesn't look nice as it could, however due lack of gatter on SSE/AVX,
      // loading values into SSE/AVX registers then performing addition produces many more
      // instructions than first adding them then putting (gathering) them into register
      for(int mu=0;mu<indexer_t::D;mu++) {
#define index_up(n,i) indexer_t::up(i[n],mu)
	int up[]  __attribute__((aligned(16))) = { REP(SIMD, index_up, indices) } ;
#define index_dn(n,i) indexer_t::dn(i[n],mu)
	int dn[]  __attribute__((aligned(16))) = { REP(SIMD, index_dn, indices) };

#define field_up_dn(n,p) p.get(up[n],k) + p.get(dn[n],k)
	small_corona += FVec(REP(SIMD, field_up_dn, field));
#define field_up_dn_2(n,p) p.get(indexer_t::up(up[n],mu),k) + p.get(indexer_t::dn(dn[n],mu),k)
	big_corona_02 += FVec(REP(SIMD, field_up_dn_2, field));

	for(int nu=0;nu<mu;nu++) {
#define index_nu(n,p) p.get(indexer_t::up(up[n],nu),k)		 \
	    + p.get(indexer_t::up(dn[n],nu),k)			 \
	    + p.get(indexer_t::dn(up[n],nu),k)			 \
	    + p.get(indexer_t::dn(dn[n],nu),k)
	  big_corona_11 += FVec(REP(SIMD, index_nu, field));
	}
      }

      FVec big_corona =
	FVec(-pars_.i_Lambda) * (big_corona_02
...
#define indexed_field(n,p,i) p.get(i[n],k)
    ...
    for(int h=0;h<N_HIT;h++) randlog[h] = RAND(tid);
    for(int h=0;h<N_HIT;h++) randlog[h] = log(randlog[h]);
	...
	new_action += corona[k] * new_phi[k];
      ...
      new_action -= (FVec(quadratic_coef) + FVec(gr)*FVec(new_phi2)) * new_phi2;
      FVec delta_action = new_action - action;

#if SIMD <= 1
      if (delta_action >= (Float)0.0 || delta_action >= randlog[h]) {
	for(int k=0;k<F::n_components;k++) phi[k] = new_phi[k];
	action = new_action;
	accepted ++;
      }
#else
      ...
      for(int k=0;k<F::n_components;k++) phi[k] = action_cond(new_phi[k], phi[k]);
    ...
    // Scatter into field
#if SIMD <= 1
    for(int k=0;k<F::n_components;k++) field.set(*indices,k,phi[k]);
#else
    for(int si=0;si<SIMD;si++) for(int k=0;k<F::n_components;k++) field.set(indices[si],k,phi[k][si]);
#endif
...
#else
    int ret = 0;
    for(int si=0;si<SIMD;si++) ret += accepted[si];
    return ret;
#endif
  }
...
template<typename F, typename P >
long int make_sweep(F &field, const parameters<Float> &pars, 
		    const P &partition ) {
...
#pragma omp for reduction(+:accepted)
    for(int s=0;s<partition.partition_size();s+=SIMD) {
      for(int si=0;si<SIMD;++si) indices[si] = partition.partition(p,s+si);
