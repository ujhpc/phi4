...
main(int argc,const char *argv[]) {
  ...
  struct poptOption options[] = {
    ...
    { "n-z", 0, POPT_ARG_INT, &n_z, 0,
      "z size" },
#endif
    { "b-x", 0, POPT_ARG_INT, &b_x, 0,
      "block x size" },
    { "b-y", 0, POPT_ARG_INT, &b_y, 0,
      "block y size" },
#if DIM >= 3
    { "b-z", 0, POPT_ARG_INT, &b_z, 0,
      "block z size" },
#endif
 ...
 fprintf(stderr,"that makes %lf ns per update\n",fnano_sec/(block_sweeps*N_HIT*(double)Ind::n_sites()*n_term));

  if(n_term>0)
    fprintf(stderr,"acceptance %f\n",((double) accepted)/(block_sweeps*N_HIT*(double)Ind::n_sites()*n_term));
  ...
  if(n_prod>0)
    fprintf(stderr,"acceptance %lf\n",
	    ((double) accepted)/(block_sweeps*(double)N_HIT*(double)Ind::n_sites()*(double)n_prod));
  //  Ind::clean();
}
